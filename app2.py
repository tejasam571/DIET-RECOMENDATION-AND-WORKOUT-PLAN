...
def recommend_meal_and_workout(user_input):
    ...
    breakfast = safe_sample(food_items, 'Breakfast')
    lunch = safe_sample(food_items, 'Lunch')
    dinner = safe_sample(food_items, 'Dinner')

    # Recommend a workout plan
    workout_plan = workout_items.sample(1) if not workout_items.empty else pd.DataFrame({})

    return {
        'BMI': bmi_pred,
        'Breakfast': {
            'FoodItem': breakfast['FoodItem'].values[0] if not breakfast.empty else 'No recommendation',
            'Calories': breakfast['Calories'].values[0] if not breakfast.empty else 'N/A',
            'Nutrients': breakfast['Nutrients'].values[0] if not breakfast.empty else 'N/A'
        },
        'Lunch': {
            'FoodItem': lunch['FoodItem'].values[0] if not lunch.empty else 'No recommendation',
            'Calories': lunch['Calories'].values[0] if not lunch.empty else 'N/A',
            'Nutrients': lunch['Nutrients'].values[0] if not lunch.empty else 'N/A'
        },
        'Dinner': {
            'FoodItem': dinner['FoodItem'].values[0] if not dinner.empty else 'No recommendation',
            'Calories': dinner['Calories'].values[0] if not dinner.empty else 'N/A',
            'Nutrients': dinner['Nutrients'].values[0] if not dinner.empty else 'N/A'
        },
        'Workout': {
            'Exercise': workout_plan['Exercise'].values[0] if not workout_plan.empty else 'No recommendation',
            'Timing': workout_plan['Timing'].values[0] if not workout_plan.empty else 'N/A'
        }
    }
